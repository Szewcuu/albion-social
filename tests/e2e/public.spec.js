...
test.describe('publiczna bramka portalu', () => {
  ...
  test('indeksuje wyłącznie publiczne strony portalu', async ({ request }) => {
    ...
    const protectedPage = await request.get('/killboard', { maxRedirects: 0 })
    ...
    expect(protectedPage.headers().location).toBe('/')
  })
  ...
  test('odrzuca anonimowe wywołania chronionych endpointów', async ({ request }) => {
    ...
    const chatPin = await request.patch('/api/chat', { data: { id: '00000000-0000-4000-8000-000000000000', pinned: true } })
    const chatDelete = await request.delete('/api/chat', { data: { id: '00000000-0000-4000-8000-000000000000' } })
    const announcementsRead = await request.get('/api/tavern/announcements')
    const announcementsWrite = await request.post('/api/tavern/announcements', { data: { title: 'Test', body: 'Test', kind: 'info', expiresInDays: 1 } })
    const announcementsDelete = await request.delete('/api/tavern/announcements', { data: { id: '00000000-0000-4000-8000-000000000000' } })
    const buildsRead = await request.get('/api/builds?category=all&limit=6')
    const buildsDelete = await request.delete('/api/builds', { data: { id: '00000000-0000-4000-8000-000000000000' } })
    ...
    expect(chatPin.status()).toBe(401)
    expect(chatDelete.status()).toBe(401)
    expect(announcementsRead.status()).toBe(401)
    expect(announcementsWrite.status()).toBe(401)
    expect(announcementsDelete.status()).toBe(401)
    expect(buildsRead.status()).toBe(401)
    expect(buildsDelete.status()).toBe(401)
