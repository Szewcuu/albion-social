...
export default function ConnectedAccountsPanel() {
  ...
  return (
    <section className="panel rounded-[26px] p-5 sm:p-7 border-[var(--border-warm)]" aria-labelledby="connected-accounts-title">
      <div className="flex flex-col justify-between gap-3 border-b border-white/8 pb-5 sm:flex-row sm:items-end">
        ...
        <span className="inline-flex items-center gap-1.5 self-start rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1.5 text-[9px] font-black uppercase text-emerald-300"><ShieldCheck className="h-3.5 w-3.5" /> Sesja chroniona</span>
      </div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        <article className="connected-account-card border-[var(--border-warm)] bg-black/25 hover:border-white/20 transition">
        ...
        {PROVIDERS.map((provider) => {
          ...
          return (
            <article key={provider.id} className="connected-account-card border-[var(--border-warm)] bg-black/25 hover:border-white/20 transition">
              <span className={`connected-account-mark ${provider.id}`}>{provider.mark}</span>
              <div><strong>{provider.name}</strong><small>{provider.detail}</small></div>
              {loading ? (
                <span className="connected-account-state"><LoaderCircle className="spin" /> Sprawdzanie</span>
              ) : isConnected ? (
                <span className="connected-account-state connected"><CheckCircle2 /> Połączony</span>
              ) : (
                <button type="button" disabled={Boolean(busy)} onClick={() => linkProvider(provider.id)} className="connected-account-connect">
                  <Link2 /> {busy === provider.id ? 'Łączenie…' : 'Połącz'}
                </button>
              )}
            </article>
          )
      ...
      <form onSubmit={updatePassword} className="account-password-form">
        <div>
          <p className="text-xs font-black text-white">Ustaw lub zmień hasło portalowe</p>
          <p className="mt-1 text-[10px] leading-5 text-[var(--text-secondary)]">Ta opcja pozwala również dotychczasowym kontom Discord logować się bezpośrednio przez e-mail.</p>
        </div>
        <label>
          <span>Nowe hasło</span>
          <span className="account-password-input">
            <KeyRound />
            <input type={showPassword ? 'text' : 'password'} autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Minimum 10 znaków" />
            <button type="button" onClick={() => setShowPassword((current) => !current)} aria-label={showPassword ? 'Ukryj hasło' : 'Pokaż hasło'}>{showPassword ? <EyeOff /> : <Eye />}</button>
          </span>
        </label>
        <label>
          ...
          <span className="account-password-input">
            <KeyRound />
            <input type={showPassword ? 'text' : 'password'} autoComplete="new-password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder="Powtórz hasło" />
          </span>
        </label>
        <button type="submit" disabled={!passwordValidation.valid || password !== confirmation || Boolean(busy)} className="btn btn-primary min-h-11 px-5 text-[10px] font-black uppercase tracking-wider">
          {busy === 'password' ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <KeyRound className="h-4 w-4" />} Zapisz hasło
        </button>
      </form>
    </section>
  )
