    ...
    <div className="guest-home">
      <section className="guest-welcome" aria-labelledby="welcome-title">
        ...
        <div className="guest-login-card auth-gateway-card">
          ...
          {mode !== 'recovery' && <><div className="auth-divider"><span>lub użyj</span></div><div className="auth-oauth-grid"><button type="button" className="auth-oauth-button google" onClick={() => startOAuth('google')} disabled={Boolean(busy)}><span className="oauth-google-mark">G</span>{busy === 'google' ? 'Łączenie…' : 'Google'}</button><button type="button" className="auth-oauth-button discord" onClick={() => startOAuth('discord')} disabled={Boolean(busy)}><span className="oauth-discord-mark">D</span>{busy === 'discord' ? 'Łączenie…' : 'Discord'}</button></div><p className="auth-discord-hint">Discord pozostaje wymagany tylko przy funkcjach komunikujących się z serwerami społeczności. Połączysz go również później w profilu.</p></>}
