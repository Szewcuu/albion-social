      ...
      <section className="auth-reset-card" aria-labelledby="new-password-title">
        ...
        {!sessionReady ? (
          ...
          <form className="auth-credentials-form" onSubmit={submit}>
            ...
            <div className="auth-password-rules"><span className={validation.checks.length ? 'valid' : ''}><Check /> 10 znaków</span><span className={validation.checks.lowercase ? 'valid' : ''}><Check /> mała litera</span><span className={validation.checks.uppercase ? 'valid' : ''}><Check /> wielka litera</span><span className={validation.checks.number ? 'valid' : ''}><Check /> cyfra</span><span className={validation.checks.special ? 'valid' : ''}><Check /> znak specjalny</span></div>
