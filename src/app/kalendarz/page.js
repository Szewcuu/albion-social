...
const TYPE_OPTIONS = ['Wszystkie typy', 'ZvZ', 'PvP', 'PvE', 'Avalon', 'Ekonomia', 'Spotkanie', 'Inne']
const ROLE_OPTIONS = [
...
export default function EventCalendarPage() {
  ...
  const filtered = useMemo(() => events.filter((event) => {
    ...
    if (type !== 'Wszystkie typy' && event.event_type !== type) return false
    ...
    <div className="page-content space-y-6">
      <header className="panel relative overflow-hidden rounded-[30px] border-amber-300/20 p-6 sm:p-8 lg:p-10">
        ...
        <div className="relative grid gap-7 lg:grid-cols-[1fr_auto] lg:items-end">
          <div><p className="text-[9px] font-black uppercase tracking-[.24em] text-violet-300">Kalendarz mobilizacji</p><h1 className="font-display mt-2 text-3xl font-black text-white sm:text-5xl">Rozkład wypraw i operacji</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--text-secondary)]">Jedno miejsce dla wydarzeń wszystkich gildii i serwerów. Wybierz rolę, zapisz się i odbierz przypomnienie w centrum powiadomień.</p></div>
...
function EventCard({ event, draft, setDraft, busy, onJoin, onCancel }) {
  ...
  return <article id={`event-${event.id}`} className="panel overflow-hidden rounded-[24px] border-violet-300/15">
    <div className="grid gap-5 p-5 sm:p-6 xl:grid-cols-[1fr_300px]">
      <div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-violet-300/25 bg-violet-300/10 px-2.5 py-1 text-[8px] font-black uppercase tracking-[.13em] text-violet-200">{event.event_type}</span><span className="rounded-full border border-sky-300/20 bg-sky-300/8 px-2.5 py-1 text-[8px] font-black uppercase tracking-[.13em] text-sky-200">{event.server}</span>{event.audience === 'guild' && <span className="rounded-full border border-amber-300/20 bg-amber-300/8 px-2.5 py-1 text-[8px] font-black uppercase tracking-[.13em] text-amber-200"><Shield className="mr-1 inline h-3 w-3" /> Tylko gildia</span>}</div><h3 className="font-display mt-4 text-2xl font-black text-white">{event.title}</h3><Link href={`/gildie/${event.guild_id}`} className="mt-1 inline-flex text-[10px] font-bold text-amber-200 hover:underline">{event.guilds?.name || 'Gildia'}</Link>{event.description && <p className="mt-3 max-w-2xl text-xs leading-6 text-[var(--text-secondary)]">{event.description}</p>}<div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-[10px] font-bold text-[var(--text-primary)]"><span className="flex items-center gap-1.5"><Clock3 className="h-4 w-4 text-amber-300" /> {formatDate(event.starts_at, { year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>{event.location && <span className="flex items-center gap-1.5"><MapPin className="h-4 w-4 text-emerald-300" /> {event.location}</span>}<span className="flex items-center gap-1.5"><Users className="h-4 w-4 text-sky-300" /> {confirmed.length}/{event.capacity}{waitlist.length ? ` · rezerwa ${waitlist.length}` : ''}</span></div></div>
      <div className="rounded-2xl border border-white/8 bg-black/20 p-4"><p className="text-[8px] font-black uppercase tracking-[.18em] text-violet-300">{joined ? 'Twój zapis' : 'Dołącz do składu'}</p><div className="mt-3 grid gap-3"><CustomSelect label="Rola" value={draft.role} onChange={(value) => setDraft({ role: value })} options={ROLE_OPTIONS} /><CustomSelect label="Przypomnienie" value={draft.reminderMinutes} onChange={(value) => setDraft({ reminderMinutes: value })} options={REMINDER_OPTIONS} /></div>{joined && <p className={`mt-3 rounded-xl border px-3 py-2 text-[9px] font-black ${isWaitlist ? 'border-amber-300/20 bg-amber-300/8 text-amber-200' : 'border-emerald-300/20 bg-emerald-300/8 text-emerald-200'}`}>{isWaitlist ? 'Lista rezerwowa' : `Potwierdzono · ${ROLE_LABELS[event.mySignup.role]}`}</p>}<div className="mt-3 flex gap-2"><button type="button" disabled={busy || !event.signup_open} onClick={onJoin} className="btn btn-primary inline-flex min-h-10 flex-1 items-center justify-center gap-2 text-[10px] font-black disabled:opacity-45">{busy ? <LoaderCircle className="h-4 w-4 animate-spin" /> : joined ? <BellRing className="h-4 w-4" /> : <Sparkles className="h-4 w-4" />}{joined ? 'Zapisz zmiany' : confirmed.length >= event.capacity ? 'Dołącz do rezerwy' : 'Zapisz się'}</button>{joined && <button type="button" disabled={busy} onClick={onCancel} className="aopp-ghost-button min-h-10 px-3 text-[9px] font-black text-rose-200">Anuluj</button>}</div></div>
    </div>
    {confirmed.length > 0 && <div className="flex flex-wrap items-center gap-2 border-t border-white/8 bg-black/10 px-5 py-3 sm:px-6"><span className="mr-1 text-[8px] font-black uppercase tracking-[.13em] text-[var(--text-secondary)]">Skład</span>{confirmed.slice(0, 12).map((signup) => <span key={signup.id} title={ROLE_LABELS[signup.role]} className="rounded-full border border-white/8 bg-black/25 px-2.5 py-1 text-[9px] text-[var(--text-primary)]">{signup.profiles?.ingame_nick || signup.profiles?.username || 'Gracz'} · {ROLE_LABELS[signup.role]}</span>)}{confirmed.length > 12 && <span className="text-[9px] text-[var(--text-secondary)]">+{confirmed.length - 12}</span>}</div>}
  </article>
}
