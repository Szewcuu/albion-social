...
function RolesPanel({ users, reason, busy, onReason, onChangeRole, onRefresh }) {
  return <section className="panel overflow-hidden"><PanelHeading eyebrow="Tylko administrator" title="Role personelu"><button onClick={onRefresh} className="btn btn-ghost btn-sm"><RefreshCw className="h-3.5 w-3.5" /> Odśwież</button></PanelHeading><div className="border-b border-[var(--border)] p-4"><label className="form-label">Powód zmiany roli<input value={reason} onChange={(event) => onReason(event.target.value)} maxLength={500} placeholder="np. przyznanie uprawnień moderatora po rekrutacji" className="mt-1.5 w-full rounded-xl border p-3 text-xs" /></label></div><div className="divide-y divide-[var(--border)]">{users.map((user) => <div key={user.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div><strong className="text-sm text-white">{user.username}</strong><p className="mt-1 font-mono text-[9px] text-[var(--text-muted)]">{user.id}</p></div><select aria-label={`Rola użytkownika ${user.username}`} disabled={busy} value={user.role} onChange={(event) => onChangeRole(user.id, event.target.value)} className="rounded-xl border px-3 py-2 text-xs"><option value="member">Użytkownik</option><option value="moderator">Moderator</option><option value="admin">Administrator</option></select></div>)}</div></section>
}
