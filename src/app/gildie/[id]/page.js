  ...
  const loadGuild = useCallback(async () => {
    ...
    try {
      ...
      const [membersResult, eventsResult, activityResult] = await Promise.all([
        ...
        supabase.from('guild_events').select('id, creator_id, title, description, event_type, starts_at, server, status, created_at').eq('guild_id', found.id).in('status', ['scheduled', 'completed']).order('starts_at', { ascending: true }).limit(30),
...
function EventCard({ event, featured = false }) {
  return <article className={`${featured ? 'mt-5' : 'panel'} rounded-[22px] border border-violet-300/15 bg-violet-300/[.035] p-5`}><div className="flex items-start justify-between gap-3"><span className="rounded-full border border-violet-300/25 bg-violet-300/10 px-2.5 py-1 text-[8px] font-black uppercase tracking-[.13em] text-violet-200">{event.event_type}</span><span className="text-[8px] font-black uppercase tracking-[.12em] text-sky-200">{event.server}</span></div><h3 className="font-display mt-4 text-xl font-black text-white">{event.title}</h3>{event.description && <p className="mt-2 line-clamp-3 text-xs leading-5 text-[var(--text-secondary)]">{event.description}</p>}<div className="mt-5 flex items-center gap-2 border-t border-white/8 pt-4 text-[10px] font-bold text-amber-200"><CalendarDays className="h-4 w-4" /> {formatDate(event.starts_at, true)}</div></article>
}
