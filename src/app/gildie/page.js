...
import { EmptyState, ErrorState, SkeletonBlock } from '@/components/ui/FeedbackState'
  ...
  return (
    <div className="page-content">
      ...
      <main className="relative z-10 mx-auto mt-2 w-full max-w-[1400px] space-y-5 p-4 sm:p-6 lg:p-8">
        ...
        {loadError ? <ErrorState icon={Shield} title="Rejestr jest chwilowo niedostępny" description={loadError} onRetry={fetchGuilds} className="panel rounded-[24px]" /> : loading ? <div className="grid gap-4 md:grid-cols-2" role="status" aria-label="Otwieranie rejestru gildii">{[0, 1].map((item) => <SkeletonBlock key={item} className="panel h-72 rounded-[24px]" />)}</div> : filteredGuilds.length === 0 ? <EmptyState icon={Swords} title="Brak pasujących chorągwi" description="Zmień kryteria albo wyczyść wszystkie filtry." action={hasFilters ? <button type="button" onClick={resetFilters} className="btn btn-ghost btn-sm">Pokaż wszystkie gildie</button> : null} className="panel rounded-[24px]" /> : <section className="grid gap-4 md:grid-cols-2">{filteredGuilds.map((guild) => <GuildCard key={guild.id} guild={guild} onInspect={() => setInspectedGuild({ name: guild.name, server: guild.server || 'Europa' })} onApply={() => setSelectedGuildForApply(guild)} />)}</section>}
